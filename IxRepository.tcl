...
####################################################################################################
# IxRepository.tcl--
#   This file implements the Tcl encapsulation of IxRepository interface for Nergear.
#
...
namespace eval IXIA {
    ...
    set stats   [list]
    set statsduration 0
    set statsRampupTime 0
    set statscount 0
    set statsnamelist {}
    set stats_info_list {}
    set stats_instant_info {}
    set NS 0
   ...
   proc loadRepository { repPath } {
      ...
      if { ![ file exists $repPath ] } {
         ...
         error "Repository(rxf) file not found..."
         #return
      } 
      set repository [ ::IxLoad new ixRepository -name $repPath ]
      Deputs "repository = $repository "
      return $repository
   }
   ...
   #       reset: whether to reset to default factory configiration
   # Return:
   #        0 if got success
...
Deputs "----- TAG: $tag -----"
   ...
   proc configObjective { actName args } {
    ...
    # Args:
    #       -repPath: the repository file absolute path
    #       -overwrite: whether override the existing file
    proc save { repPath } {
        ...
        return [ $repository write -destination $repPath -overwrite 1]
    }
    ...
    proc apply {} {
        ...
        $testController applyConfig $activeTest
    }
    ...
    proc run {} {
        ...
        set chassisChain [ $repository cget -chassisChain ]
        $chassisChain refresh
        set repName	[ $repository cget -name ]
        set activeTest    [ getActiveTest ]
        set testName	[ $activeTest cget -name ]
        
        $testController setResultDir "Result/${repName}/${testName}"
        $activeTest config \
...
Deputs "stats:$stats"
			...
			if { [ info exists StatsList($stats) ] } {
				lappend proIndexList $StatsList($stats)
			} else {
...
Deputs "aggregationType:$aggregationType"
		...
		${NS}::StartCollector -command IXIA::collectStats -interval $interval
		incr count
    }
    ...
    proc statsCal { proIndexList  } {
        ...
        if { $startIndex == 0 && $endIndex == 1 } {
	          ...
	          set start [lindex $stats_info_list $startIndex]
            set  resultValueList {}          
...
Deputs "full stats info:$args"    
		...
		Deputs "stats_info: $stats_info_list"          
   ...
   ####################################################################################
    ...
    proc getInstantStats { proItem } {
		...
		if { [ string is integer $proItem ] == 0 } {
			set proItem [ lsearch $SelectedStats $proItem ]
			if { $proItem == "-1" } {
				return ""
			}
		} else {
		...
		set statsIndex [ lindex $stats_info_list $proItem ]
